...
\begin{frame}{The Goal}
    \begin{itemize}
        \item<1-> \textit{\color<2->{purple}Identification} \\
            \only<2->{
                Naive causal identification with ordinal outcome "index" fails \\
                \vspace{0.2cm}
...
\begin{frame}{Identification Through Link Function}
    \begin{itemize}[<+->]
        \item Cumulative probabilities such as \textcolor{orange}{$\P{Y \le j}$}
        \item If we can \textit{map} this probability to the $Y^{\ast}$ space
    ...
    \begin{itemize}[<+->]
        \item One caveat: for a positive constant $C$,
            ...
            \begin{aligned}
                ...
                &= \P{C\cdot \tau D_{i} + C \cdot \varepsilon_i \le C \cdot \alpha_{4}} \\
            \end{aligned}
            $$ 
...
\begin{frame}{How Can We Estimate?}
    \begin{itemize}[<+->]
        ...
        \item They assume $F_{\varepsilon_i}$ is a specific distribution $\rightarrow$ MLE

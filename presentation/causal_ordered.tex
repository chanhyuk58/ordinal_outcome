...
\begin{frame}{The Goal}
    \begin{itemize}
        ...
        \item<1-> \textit{\color<3->{Orange}Estimation} \\
            \only<3->{
                Parametric ordered probit and logit require strong distributional assumptions \\
                \vspace{0.2cm}
...
\begin{frame}[label=main_cardinal]{Why Naive Identification Fails}
    ...
    \vspace{0.2cm}
    \begin{itemize}
        \item<1-> $\tau = \E{Y^{\ast}(1)} - \E{Y^{\ast}(0)} \approx \textbf{+0.2}$ 
        \only<3>{
            \item $\tau_{a} = \E{Y_{a}(1)} - \E{Y_{a}(0)} \approx \textbf{+0.4}$
        }
        \only<5>{
            \item $\tau_{b} = \E{Y_{b}(1)} - \E{Y_{b}(0)} \approx \textbf{-0.2}$
        }
    \end{itemize}
    \only<5>{
...
\begin{frame}{Identification Through Link Function}
    \begin{itemize}[<+->]
        \item $Y^{\ast}_{i} = \tau D_{i} + \varepsilon_i$
        \item Cumulative probabilities such as \textcolor{orange}{$\P{Y \le \text{Agree}}$}
        \item If we can \textit{map} this probability to the $Y^{\ast}$ space $\rightarrow$ Identify $\tau$!
        \item \textcolor{Green}{Link Function = CDF of $\varepsilon_i$ ($F_{\varepsilon_{i}}$)} does this:
            $$
                F_{\varepsilon_{i}}^{-1}\bigl(\textcolor{orange}{\P{Y_{i} \le \text{Agree}}}\bigl) = \underbrace{\alpha_{4} - \tau D_{i}}_{\text{Defined in $Y^*$ space}}
            $$
    ...
    \end{itemize}
    \only<5>{
        \hfill\hyperlink{appx_id}{\beamergotobutton{More on Identification}}
    }
\end{frame}

\begin{frame}{New Estimands: Normalized Treatment Effect}
    \begin{itemize}[<+->]
        \item Problem is, for a positive constant $C$,
            $$
            \begin{aligned}
                \P{Y^{\ast}_{i} \le \alpha_{4}}
                &= \P{C\cdot Y^{\ast}_{i} \le C \cdot \alpha_{4}} \\
            \end{aligned}
            $$ 
        \item Thus $\tau$ becomes indistinguishable from $C \cdot \tau$
...
\begin{frame}{How Can We Estimate?}
    \begin{itemize}
        \item The key is \textcolor{Green}{Link Function ($F_{\epsilon_{i}}$)}
        \item Ordered Probit, Ordered Logit, and most IRT models \\
        \item Assume $F_{\varepsilon_i}$ is a specific distribution $\rightarrow$ MLE \pause
        \item Distributional Assumptions can be violated
            \begin{itemize}
                \item Pure misspecification
                \item Unaccounted covariate
            \end{itemize} \pause
            $\implies$ Become \textit{\textcolor{red}{inconsistent}} \pause
            \vspace{0.3cm}
...
\begin{frame}{Simulation Results: Bias and MC SD}
    \centering
...
\begin{frame}[label=appx_id]{More on Identification}
    \begin{tikzpicture}[font=\sffamily, thick, scale=1.1]
        \only<2-6>{
            \centering
            \begin{scope}
    ...
    \begin{itemize}[<+->]
        ...
        % \item If we can \textit{link} this probability to the $Y^{\ast}$ space $\rightarrow$ Identify $\tau$!

    ...
    \only<3->{
        \begin{tikzpicture}[font=\sffamily, thick]
            ...
            \begin{scope} 
                ...
                % Axis Label
                \node[above, yshift=5pt] at (5, 0) {\scriptsize\textbf{The True Outcome}};
            \end{scope}
            ...
            \only<4>{
                \begin{scope} 
                    ...
                    % Bottom Labels
                    \node at (1, -1.5) {\scriptsize\itshape Strongly Disagree};
                    \node at (3.25, -1.5) {\scriptsize\itshape Disagree};
                    \node at (5.25, -1.5) {\scriptsize\itshape Neither};
                    \node at (7.25, -1.5) {\scriptsize\itshape Agree};
                    \node at (9.25, -1.5) {\scriptsize\itshape Strongly Agree};
                \end{scope}
            }
            ...
            \only<5>{
                \begin{scope} 
                    ...
                    % Bottom Labels
                    \node at (1.5, -1.5) {\scriptsize\itshape Strongly Disagree};
                    \node at (3.5, -1.5) {\scriptsize\itshape Disagree};
                    \node at (4.5, -1.5) {\scriptsize\itshape Neither};
                    \node at (7, -1.5) {\scriptsize\itshape Agree};
                    \node at (9.5, -1.5) {\scriptsize\itshape Strongly Agree};
                \end{scope}
            }
        \end{tikzpicture}
    }
...
\begin{frame}{Why Naive Identification Fails}
    ...
    \centering
        \begin{tikzpicture}[font=\sffamily, thick]
            ...
            \begin{scope} 
                ...
                % Axis Label
                \node[above, yshift=5pt] at (5, 0.5) {\scriptsize\textbf{Preference over Political System ($\boldsymbol{Y^{\ast}}$)}};
                % Top Labels
            ...
            \only<3>{
                ...
                \begin{scope}[purple]
                    ...
                    % Bottom Labels
                    \node at (1, -1.5) {\scriptsize\itshape Strongly Not Prefer};
                    \node at (3.25, -1.5) {\scriptsize\itshape Not Prefer};
                    \node at (5.25, -1.5) {\scriptsize\itshape Neutral};
                    \node at (7.25, -1.5) {\scriptsize\itshape Prefer};
                    \node at (9.25, -1.5) {\scriptsize\itshape Strongly Prefer};
                \end{scope}
            }
    ...
    \pause
    \begin{itemize}
        \item The treatment effect we want: $\E{Y^{\ast}(1)} - \E{Y^{\ast}(0)} = \tau^{\ast}$
        \item<3, 5> \textit{\color{purple} Unknown Reporting Function $g$}
        \item<4-> \textit{\color{blue} Arbitrary Numerical Index $f$}
        \item<5-> What we get: $\E{{\color{blue}f}({\color{purple}{g}}(Y^{\ast}(1)))} - \E{{\color{blue}f}({\color{purple}{g}}(Y^{\ast}(0)))} \neq \tau^{\ast}$
    \end{itemize}
\end{frame}

\begin{frame}{New Estimands: Normalized Treatment Effect}
    \begin{itemize}[<+->]
        \item Nonparametrically identified probability distribution of each answers \\
            $\implies$ identify the Treatment Effect up to scale \\ 
    ...
    \begin{itemize}
        ...
        \item Distributional Assumptions can be violated
            \begin{itemize}
                ...
                \item Unaccounted confounder
            \end{itemize} \pause
            \vspace{0.2cm}
        \item Become \textit{inconsistent} \pause
            \vspace{0.3cm}
        \item \textcolor{Green}{Two Flexible Estimators} without rigid Distributional Assumptions
    \end{itemize}
\end{frame}
...
\begin{frame}{Alternative: Estimate the Distribution}
    \begin{itemize}
        ...
        \item<1-> Parametric Generative Model: \textcolor{Orange}{Normalizing Flows}
            ...
            \vspace{0.5cm}
        \item<1, 4-> Plug into MLE
            $$ 
                \ell(\alpha,\beta,\tau) = \sum_{i=1}^n \sum_{j=0}^J 1_{{Y_i=j}} \log\left[ \hat{F}_{\varepsilon_i}\bigl( D_i, \tau_i \bigr)
                \hat{F}_{\varepsilon_i}\bigl( D_i, \tau_i \bigr) \right] 
            $$ 
...
\begin{frame}{Replication -- \citet{Mattingly2025a}}
    \begin{itemize}
        \item Effect of promotion videos on preference over political system
        \item \textit{Strongly Prefer the US} to \textit{Strongly Prefer China}
    \end{itemize}
...
\begin{frame}{Conclusion}
    \begin{itemize}
        \item<1-> \textit{\color<2->{purple}Identification} \\
            \only<2->{
                Naive use of ordinal "index" is not what we want \\
                \vspace{0.2cm}
                $\implies$ \textcolor{purple}{Normalized Latent Treatment Effect}
            }
            \vspace{0.5cm}
        \item<1-> \textit{\color<3->{Orange}Estimation} \\
            \only<3->{
                Standard ordinal regressions risk inconsistency \\
                \vspace{0.2cm}

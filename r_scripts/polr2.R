...
polr <-
    function(formula, data, weights, start, ..., subset,
             ...
             method = c("logistic", "probit", "loglog", "cloglog", "cauchit", "kde", "ks"))
{
    ...
    if(missing(start)) {
        ...
        fit <-
            switch(method,
                   ...
                   "kde" = glm.fit(X, y1, wt, family = binomial(), offset = offset),
                   "ks" = glm.fit(X, y1, wt, family = binomial(), offset = offset))
        if(!fit$converged)
        ...
        # the first values are the guesses for the coefficients
        # the later values are the guesses for the split points

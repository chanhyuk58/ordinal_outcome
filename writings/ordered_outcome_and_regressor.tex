...
$D$ simulates treatment status, so each data point is randomly assigned $0$(control) or $1$(treatment), $X$ is drawn from $\mathcal{N}(0, 5)$ and $\tau$ set to be $0.5$. 
...
For the OLS, a cardinalization of $Y$ is required. Here I used two different cardinalization scheme to demonstrate that arbitrary cardinalization can lead to very different conclusions. The first cardinalization follows the commonly used practice with labels of $\{1, 2, 3\}$. The second cardinalization multiplies the first labels by 10 times, $\{10, 20 ,30\}$. The second cardinalization would effectively increase the size of the estimated causal effect compared to that from the first cardinalization. 

I tested for two different distribution for the error term, $\varepsilon$, the standard normal distribution $\mathbb{N}(0, 1)$ and the t-distribution with degrees of freedom of 1. The t-distribution has heavier tail regions than both the standard logistic and the standard normal distribution. Thus, by construction, the normal error represents the most favorable DGP for the ordered probit model, and the t-distribution error term case represents DGP where the distributional assumptions for both ordered logit and ordered probit models are violated. Four sample sizes of 250, 500, 750 and 1,000 are considered. I first generate a population of size $1e+5$ and draw random samples for Monte Carlo replication from this population. The number of Monte Carlo replication is 1,000, and all estimations are the means of these 1,000 replication results. Regarding the KDE-based estimator, I set $\delta = \frac{1}{6}$, pilot bandwidth $h_{p} = n^{-0.11}$, the final base bandwidth $h_{f} = n^{-0.15}$, and fix all other parameters to the middle points of the suggested intervals. For instance, $\epsilon$ in the damping function is set to be $\frac{0 + (\frac{1}{40}- \frac{\delta}{20})}{2} \approx 0.33$. Also, the trimming was done at the 0.05 level, so data points within 0.05 quantile and 0.95
...
First, in terms of root mean squared error (RMSE), Panel (b) shows that all models are comparably doing well, meaning that as the sample size increases all the models converges to certain values. Panel (a) shows the mean bias from 10,000 simulations of two OLS, ordered logit model, ordered probit model, and KDE-based estimator. The one notable thing is that OLS with common cardinalization of $\{1, 2, 3\}$ is significantly underestimating the $\tau$. However, OLS may also overestimate the effect if other cardinalization scheme is used, and this is shown by the result from OLS with $\times 10$ model. OLS with $\{10, 20, 30\}$ cardinalization would blow up the coefficient and thus lead to overestimation. Thus, the results from OLS models are highly dependent on the cardinalization schemes, and thus unreliable.

Both ordered probit model and ordered logit model show low estimated bias, but KDE-based estimator shows much smaller bias across all sample sizes. Considering that the standard normal error case meets the distributional assumption for ordered probit model, the great performance of the ordered probit model is expected. Also, the CDF of standard logistic is not too different from that of standard normal, ordered logit model also performs comparably well. However, KDE-based approach shows almost 0 bias across the sample sizes, performing the best.
...
Figure 2 describes the results from the t-distribution error term case. Again, two OLS shows large bias and depending on the cardinalization scheme, the bias can be positive and negative. Ordered logit and ordered probit model also suffers some bias, while KDE estimator shows the best performance.

% These are somewhat expected from the previous diagnosis using surrogate-based residuals. Since the models cannot aptly capture the true error distribution, the coefficient estimates become biased and inconsistent. Considering the standard errors, these coefficients would not carry statistically significant meanings. Klein and Sherman estimator generally does well, although it overestimates the coefficients of the treatment variable and fails to achieve statistically significant results. 

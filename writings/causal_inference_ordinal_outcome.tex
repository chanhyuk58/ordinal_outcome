...
To overcome this shortcoming, this paper introduces alternative semiparametric modeling based on Kernel Density Estimation (KDE) in the next Section. The novelty of the semiparametric approach comes from it relaxes the distributional assumption, and estimate the error distribution directly from the data.
...
\section{Semiparametric Approach}
\subsection{KDE-based Semiparametric Ordinal Regression}
...
\end{assumption}
{\color{blue}
    Thus, it is advised to include (continuous) covariates in the estimation process to correctly estimate the error distribution. If they are not included, the error term may not be correctly estimated due to the lack of support (violation of the differentiable conditional density assumption). Also, this make the estimation inefficient since variations from other covariates will be absorbed to the error, making the shape of the error more complex.


    \textit{Alternative}:
    I may add innocuous continuous random variable(s) to the estimation process to ensure this property. That said, add $X_{inno}$ such that $\beta_{inno} = 0$. This make the estimation less efficient, but may be useful.
}
...
The estimation of $g_{1}(\cdot)$ starts with pilot density estimation. Let $\hat{\sigma}_{1}$ be the sample standard deviation of the $V_{k} = V \cdot 1_{\{ Y_{k} \le j \}}$ and $n_{1} = \sum_{k} 1_{\{ Y_{k} \le j \}}$. Fix $\delta \in (0, \frac{1}{3})$, and define a pilot bandwidth $h_{p} = n^{-\gamma}$, where $\frac{1}{10} < \gamma < \frac{1}{3(3 + \delta)}$. Then the pilot KDE of $g_{1}(\cdot)$ to be:
...
Choose $ \psi \in (\frac{3 + \delta}{20}, \frac{1}{6})$. Let $ h_{f} = n^{-\psi}$, the base final bandwidth. Note that this base final bandwidth is designed to be smaller than the pilot bandwidth, since while capturing the whole data points is more important in pilot estimation, the final bandwidth should be small enough to capture more subtle details. Now calculate $\hat{g}_{1}(\cdot)$ based on above values:
...
\begin{theorem}[Consistency of the estimator]
    For an consistent KDE, as $n \to \infty$, $| \hat{\tau} - \tau | = o_{p}(1)$
\end{theorem}
...
\subsection{Relaxing the Common Reporting Function Assumption}

As briefly mentioned in Section 2, we can relax the common reporting function assumption that every respondents share a internal process of converting their latent preference to the ordinal categories by making the reporting function, $g$, depend on individual's observed characteristics $X$. More explicitly, we can model this heterogeneity as the thresholds linearly depend on $X$.

\begin{assumption}{Heterogeneous Reporting Function}
    Let $X$ be a vector capturing the observed individual characteristics. Then, we can assume that the reporting function $g$ varies across $X$:
    $$
        Y = g(Y^{\ast}) = \begin{cases}
        0 & \text{if } \gamma_{-1}X < Y^{\ast} \le \gamma_{0}X \\
        1 & \text{if } \gamma_{0}X < Y^{\ast} \le \gamma_{1}X \\
        \vdots & \vdots  \\
        J & \text{if } \gamma_{J-1}X < Y^{\ast} \le \gamma_{J}X \\
    \end{cases}
    $$, where $\gamma_{j}$ captures how $X$ shifts the thresholds.
\end{assumption}

In the estimation process, this can be taken account by allowing different $\beta_{j} = \gamma_{j} + \beta$ for each ordinal category $j$. Then plugging in $V_{j} = f(X, \beta_{j}) + \epsilon_{j}$ instead of $V = f(X, \beta) + \epsilon$ will give the similar results for the above. This also can be viewed as a partial proportional odds model \citet{Williams2006a}, where the proportional odds assumption holds for treatment assignments and does not for the rest of the covariates. {\color{blue} need explanation on proportional odds assumption?}. One caveat is that since now the model has to estimate more coefficients, larger sample size is desirable.


{\color{blue} 

I am not sure how much should I invest on this. Since we are assuming that the treatment assignments would not affect the reporting function, whether we consider this heterogeneity or not will not bias the coefficients of the treatment assignments (but may increase the efficiency). Maybe critical to cardinalization..


\subsection{Normalizing Flows -- Potential alternative to KDE}
Normalizing flows are generative models which produce tractable distributions where both sampling and density evaluation can be efficient and exact, using invertible functions. In other words, it is a technique that maps a random variable with a complex probability density function to the simpler density function.
...
$$
\begin{aligned}
    P_{\mathbf{Y}}(\mathbf{y}) &= P_{\mathbf{Z}}(\mathbf{f}(\mathbf{y}))| \text{det} \D\mathbf{f}(\mathbf{y}) | \\
           &= p_{\mathbf{Z}}(\mathbf{f}(\mathbf{y}))| \text{det} \D\mathbf{g}(\mathbf{f}(\mathbf{y})) |^{-1},
\end{aligned}
$$
where $\mathbf{g}$ is an invertible function, $\mathbf{f} = \mathbf{g}^{-1}$, and $\D\mathbf{f}(\mathbf{y}) = \frac{\partial \mathbf{f}}{\partial \mathbf{y}}$ is the Jacobian of $\mathbf{f}$. Since $\mathbf{f}$ moves in the direction of a simple base distribution, it is called \textit{normalizing flow}. 
...
\subsection{Normalizing Flows based Estimator}

Big picture: Surrogate based approach -- idea from \citet{Liu2018a}

Goal of the Estimation: Maximize the below log likelihood
$$
\begin{aligned}
    \frac{1}{n}\sum_{i=1}^{n} \sum_{j=0}^{J} \log \left[ F(\alpha_{j} - tD - X\beta) - F(\alpha_{j-1} - tD - X\beta) \right] \\
    = \frac{1}{n}\sum_{i=1}^{n} \sum_{j=0}^{J} \log \left[ \int_{\alpha_{j-1} - tD - X\beta}^{\alpha_{j} - tD - X\beta} F(\varepsilon) d\varepsilon \right] \\
\end{aligned}
$$ 

We can model $F$ with a normalizing flow as $F$ follows same distribution as $f(\cdot; \theta)$, where $\theta$ is a set of parameters for the normalizing flow. Approximate the integral by resampling \textit{surrogates} for $Y^{\ast}$ from the $f(\cdot; \theta_{old})$ and $\hat{\beta}_{old}$ $\hat{\alpha_{old}}$, and then update the flow based on the residuals calculated from the surrogates. Unlike KDE, normalizing flows allows resampling from a learned distribution, thus this is possible. Based on the updated flow, optimize to find $\hat{\beta}_{new}$ and $\hat{\alpha}_{new}$. Iterate the process.


Pros: if we set the base distribution for the flow as standard normal, then we may map the coefficient estimate back to the standard normal. This can be interpreted as a normalizing by the standard deviation of the error term (since standard normal has sd of 1). Consistency guaranteed, since normalizing flows based on MLE. 

Cons: Computationally costly compared to KDE. Hyperparameter tuning. Python.

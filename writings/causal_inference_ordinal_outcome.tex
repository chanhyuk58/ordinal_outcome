...
\begin{abstract}
    Causal inference has become central to political science, yet a large share of empirical work relies on ordinal outcomes such as Likert-type survey responses or ratings. Standard approaches to these outcomes -- cardinalization, IRT based approach, or estimating ordered logit and probit models -- raise fundamental problems. Cardinalization introduces arbitrariness, IRT based approach is useful, but usually not available, and ordered regression models depend on strict distributional assumptions, leading to bias and potentially misleading substantive conclusions. This paper clarifies the limits of causal identification with ordinal outcomes, showing that treatment effects on the latent scale are only identifiable up to scale, even under common assumptions for causal inference. To address the shortcomings of existing methods, I propose a semiparametric kernel density estimation (KDE)-based estimator that avoids strong distributional assumptions while retaining interpretability. Monte Carlo simulations demonstrate that the KDE estimator remains unbiased across a wide range of error distributions, whereas OLS and ordered regression models fail under misspecification. An empirical application to survey data replicates results from Tomz (2020) and reveals that the proposed estimator produces substantively different conclusions, highlighting the risks of relying on conventional methods. These findings suggest that semiparametric approaches provide a more robust path forward for causal inference with ordinal outcomes in political science.
\end{abstract}
...
To be more concrete, let us suppose a following simple model. Suppose a latent continuous outcome variable of $Y^{\ast}$. This can be thought as lining up people based on their preferences or opinion on a certain political issue.  Then, assume following partially linear true data-generating process (DGP):
\begin{equation}
    Y^{\ast} = f(X, \beta) + D^{T}\tau + \varepsilon
\end{equation}
$D$ is a binary treatment indicator, where $1$ denotes a treatment group and $0$ denotes a control group, $X$ is a vector of regressors, and $\varepsilon$ is a latent error term and its density is defined with $f$, and its distribution is defined by $F$. 

One natural causal estimand is the average treatment effect (ATE) in terms of $Y^{\ast}$, which equal to $\tau$ in Equation 1. Borrowing from the potential outcome framework \citep{Rubin1974a}, let us denote the potential outcome in the latent space as $Y^{\ast}(d)$, where $d \in \{0, 1\}$ denotes the treatment status. Then the ATE can be expressed as the difference between the expectations of the two potential outcomes:
$$
    \mathbb{E}\left[Y(1)\right] - \mathbb{E}\left[Y(0)\right]
$$ 

Now we put following standard causal inference assumptions regarding the latent $Y^{\ast}$. 
...
If all above assumptions are met and if we can directly observe the $Y^{\ast}$, ATE on $Y^{\ast}$ can be written as 
$$
    \mathbb{E}\left[Y(1)\right] - \mathbb{E}\left[Y(0)\right] = \mathbb{E}\left[Y^{\ast}_{i}(1) \mid D_i = 1 \right] - \mathbb{E}\left[Y^{\ast}_{i}(0) \mid D_i = 0\right] = \tau
$$. 

The problem is that we usually can not observe the continuous $Y^{\ast}$ directly, but only can observe the ordinal outcomes, the transformed version of $Y^{\ast}$. Denote the observed ordinal outcome as $Y$ and the transformation function (often called reporting function) as $g: \mathbb{R} \to \{1, 2, 3, \ldots, J\}$, which also cannot be observed directly.
...
Under this setting, we only can observe $Y$ directly, and have no good knowledge on either $Y^{\ast}$ nor $g$. Thus, the naive estimator of ATE, $\mathbb{E}\left[Y^\ast(1)\right] - \mathbb{E}\left[Y^\ast(0)\right]$ cannot be obtained. Also, since $g$ is unknown and scaling by positive constant would not change the ordering of $Y$, so we only can identify the target causal parameter up to scale. In other words, we only can identify the size of the treatment effect relative to a coefficient of another covariate. Let us denote $\beta_{a}$ as a coefficient of a covariate that is going to work as an anchor. Then we can define the anchored latent treatment effect as:
...
\begin{assumption}{Distributional Assumption on Error Term}
     The error term, $\varepsilon$ follows certain distribution. For example, ordered logit model assumes standard logistic distribution and ordered probit model assumes standard normal. 
\end{assumption}
...
\begin{aligned}
    ...
    &=\mathbb{P}\left(\varepsilon \le \alpha_j - (X_{i}^{T}\beta + D_{i}^{T}\tau) \right) - \mathbb{P}\left(\varepsilon > \alpha_{j-1} - (X_{i}^{T}\beta + D_{i}^{T}\tau) \right)\\
...
$$
    \hat{g}_{1}(\cdot) = \frac{1}{n_{1}} \sum_{k} 1_{\{ Y_{k} \le j \}} \cdot K \left( \frac{(X_{i}^T\beta + D_{i}^T \tau) - f(X_{k} - \beta)}{\hat{w}_{1i} \cdot h_{f} }\right) \cdot \frac{1}{\hat{w}_{1i} \cdot h_{f} } 
$$ 
...
Even if we used local smoothing technique in estimation of the $\hat{\mathbb{P}}$, there is still a concern that the some edge cases of $V$ may drag the estimation. In this situation trimming out some extreme data points can be a solution. Trimming function $\hat{m}$ does this by trimming out the data points whose absolute value are not within the range of $q$th quantile of $|X_{i}|$. 
$$

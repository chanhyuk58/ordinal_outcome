...
#{{{
# Load data
df = pd.read_stata("~/Dropbox/projects/ordinal_outcome/replications/Carlin2021a/study1_2.dta")
df = df[(df['tasa2'] != 88) & (~df['arm'].isin([2, 4]))]

# Select columns for X and y from the DataFrame
y_pd = df["tasa2"]
X_pd = df[["arm3", "arm5", "arm6", "arm7", "arm8"]]
...
summary.to_csv("../data/carlin_nf(2).csv", index=False, encoding="utf-8-sig")

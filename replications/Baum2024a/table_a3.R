...
# raw <- read_dta('./November Data File (for replication).dta')
df <- read_dta('./November_Data_File_recoded.dta')
df$violence051 <- factor(df$violence051)
df$el_fair_4 <- ordered(df$el_fair_4)
names(df)

# design <- svydesign(ids = ~1, weights = ~weight_nat, data = df)
# # Fit model using survey-weighted ordered logistic regression
# model <- svyolr(factor(violence051) ~ phq9 + misperceptionssum12 + phq9_fn + anypolactive +
#                   dem + rep + ind + libcon + interest + black + white + asian + 
#                   hispanic + income_r + edu_r + age + el_conf + male + trumpsupporter,
#                 design = design)
# summary(model)

model <- polr(violence051 ~ phq9 + misperceptionssum12 + phq9_fn + anypolactive +
                dem + rep + ind + libcon + interest + black + white + asian + 
                hispanic + income_r + edu_r + age + el_conf + male + trumpsupporter,
              data = df, method = "logistic", Hess=TRUE, weights=weight_nat)

model2 <- polr(el_fair_4 ~ phq9 + misperceptionssum12 + phq9_fn + anypolactive +
                dem + rep + ind + libcon + interest + black + white + asian + 
                hispanic + income_r + edu_r + age + el_conf + male + trumpsupporter,
              data = df, method = "logistic", Hess=TRUE, weights=weight_nat)
...
Y <- df$violence051
X <- df[, c(
'phq9', 'misperceptionssum12', 'phq9_fn', 'anypolactive',
                'dem', 'rep', 'ind', 'libcon', 'interest', 'black', 'white', 'asian', 
                'hispanic', 'income_r', 'edu_r', 'age', 'el_conf', 'male', 'trumpsupporter'
)]

comidx <- complete.cases(
  df[, c(
    'el_fair_4', 'phq9', 'misperceptionssum12', 'phq9_fn', 'anypolactive',
    'dem', 'rep', 'ind', 'libcon', 'interest', 'black', 'white', 'asian', 
    'hispanic', 'income_r', 'edu_r', 'age', 'el_conf', 'male', 'trumpsupporter'
  )]
)
...
fit_ks <- ks_estimator(Y, X, gweights=data.frame(df, drop=TRUE)[, 'weight_nat'][comidx])
# gweights <- data.frame(df, drop=TRUE)[, 'weight_nat'][comidx]
fit_ks[[1]]
fit_ks[[2]]
...
write.csv(ks_result, file='./ks_result_discretize.csv')
